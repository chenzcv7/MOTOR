...
class BLIP_Pretrain(nn.Module):
    def __init__(self,                 
        ...
        med_config = 'configs/bert_config_sci.json'
        encoder_config = BertConfig.from_json_file(med_config)
        encoder_config.encoder_width = vision_width
        self.text_encoder = BertModel.from_pretrained('allenai/scibert_scivocab_uncased',config=encoder_config, add_pooling_layer=False)
        ...
        self.label_list = ['Atelectasis', 'Cardiomegaly', 'Consolidation', 'Edema', 'Enlarged Cardiomediastinum',
    ...
    def forward(self, image, caption, knowledge_gk, knowledge_sk, label, alpha):
        ...
        text = self.tokenizer(caption, padding='max_length', truncation=True, max_length=90,
        ...
        label_inputs_ids = torch.cat([self.tokenizer(f"{c}", padding='max_length', truncation=True, max_length=5,
                                                return_tensors="pt").input_ids for c in self.label_list]).to(image.device)
        label_inputs_mask = torch.cat([self.tokenizer(f"{c}", padding='max_length', truncation=True, max_length=5,
                                                 return_tensors="pt").attention_mask for c in self.label_list]).to(image.device)

        label_outputs = self.text_encoder(label_inputs_ids, attention_mask = label_inputs_mask,
                                        return_dict = True, mode = 'text')
        label_features = F.normalize(self.text_proj(label_outputs.last_hidden_state[:,0,:]),dim=-1) # 14 x 768

        image_label_similarity = F.sigmoid((100.0 * torch.bmm(image_feat.unsqueeze(1), label_features.unsqueeze(0).repeat(image_feat.size(0),1,1).permute(0,2,1))).squeeze(1))
        ...
        return loss_ita, loss_itm, loss_lm, loss_mlc
