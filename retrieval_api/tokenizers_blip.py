...
class Tokenizer(object):
    def __init__(self, args):
        if args.dataset_name == 'iu_xray':
            self.ann_path = args.ann_path.split('&')[0] + 'train.json'
        ...
        self.special_id = [19, 20, 21, 23]

    def create_vocabulary(self):
        total_tokens = []
        # special_words = 'picture picture picture.'
        # special = {'report': special_words}
        # self.ann['train'].append(special)
        for cur_ann in self.ann:
            report = cur_ann['caption']
        # for report in self.ann.loc[self.ann['Split'] == 'train', 'Report']:
        ...
        vocab = [k for k, v in counter.items() if v >= self.threshold] + ['<unk>'] + ['[DEC]'] + ['<pad>'] + ['<cls>'] + ['<sep>'] + ['[ENC]']
        vocab.sort()
        token2idx, idx2token = {}, {}
