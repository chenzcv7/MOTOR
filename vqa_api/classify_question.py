...
import torch
from dataset_RAD_new import Dictionary,VQAFeatureDataset
...
class QuestionAttention_new(nn.Module):
    def __init__(self, dim):
        super().__init__()

        self.tanh_gate = linear(768 + dim, dim)
        self.sigmoid_gate = linear(768 + dim, dim)
        self.attn = linear(dim, 1)
        self.dim = dim

    def forward(self, context, question):  #b*12*768 b*12*1024

        concated = torch.cat([context, question], -1)  # b * 12 * 300 + 1024
        concated = torch.mul(torch.tanh(self.tanh_gate(concated)), torch.sigmoid(self.sigmoid_gate(concated)))  #b*12*1024
        a = self.attn(concated) # #b*12*1
        attn = F.softmax(a.squeeze(-1), 1) #b*12

        ques_attn = torch.bmm(attn.unsqueeze(1), question).squeeze() #b*1024

        return ques_attn

class typeAttention_new(nn.Module):
    def __init__(self):
        super(typeAttention_new, self).__init__()

        self.q_final = QuestionAttention_new(1024)
        self.f_fc1 = linear(1024, 2048)
        self.f_fc2 = linear(2048, 1024)
        self.f_fc3 = linear(1024, 1024)
...
class typeAttention_new2(nn.Module):
    def __init__(self):
        super(typeAttention_new2, self).__init__()

        self.q_final = QuestionAttention_new(768)
        self.f_fc1 = linear(768, 768 * 2)
        self.f_fc2 = linear(768 * 2, 768)
        self.f_fc3 = linear(768, 768)
