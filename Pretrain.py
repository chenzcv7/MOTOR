...
def main(args, config):
    ...
    datasets = [create_dataset('pretrain', args, config)]
    ...
    data_loader = create_loader(datasets, samplers, batch_size=[args.batch_size], num_workers=[4], is_trains=[True],
                  collate_fns=[None])[0]

    print("Creating model")
    model = blip_pretrain(image_size=config['image_size'], vit=config['vit'], vit_grad_ckpt=config['vit_grad_ckpt'],
                          vit_ckpt_layer=config['vit_ckpt_layer'], queue_size=config['queue_size'], args=args)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--ann_path', type=str,
                        default='./dataset/KG/mimic_train_kg_AO.json',
                        help='the path to the directory containing the data.')
